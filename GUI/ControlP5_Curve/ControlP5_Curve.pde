...
void setup() {
  ...
  myChart2.setData("incoming", new float[400]);
...
void draw() {
  ...
  myChart2.unshift("incoming", (noise(frameCount*0.1)*20));
  
  // push: add data from right to left (last in)
  myChart.push("incoming", (noise(frameCount*0.1)* 20 ));
}
