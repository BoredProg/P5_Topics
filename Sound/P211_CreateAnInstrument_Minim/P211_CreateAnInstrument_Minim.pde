...
void setup()
{
  ...
  out.playNote( 0.5, 0.9, new SineInstrument( 123.47 ) );
  
  // we can use the Frequency class to create frequencies from pitch names
  out.playNote( 1.0, 0.9, new SineInstrument( Frequency.ofPitch( "C3" ).asHz() ) );
  out.playNote( 1.5, 1.9, new SineInstrument( Frequency.ofPitch( "E3" ).asHz() ) );
  out.playNote( 2.0, 2.2, new SineInstrument( Frequency.ofPitch( "G3" ).asHz() ) );
}
