class Ribbon
{	
	private GLGraphicsOffScreen offScreen = null;
	private boolean offScreenEnable = false;
	...
	public void Update( PVector pos )
	{
		...
		for( int i = 1; i < ptCnt-1; i++ ) rPt[i].CalcPoints( i, rPt[i-1], rPt[i+1] );
	}
	
	
	public void Render()
	{
		gl.glColor4f( red(col), green(col), blue(col), 1.0 );
